...
Committing early and often is encouraged. In general you should not check in 
...
`all_shapes.scad`. At this time the code should print 4 messages starting with
"Something's wrong with ...". You will fix these later by implementing the lab
functions.

### OpenSCAD

[OpenSCAD](http://www.openscad.org/documentation.html) is a Constructive
Solid Geometry (CSG) language with an integrated development environment and
viewer. Double clicking files with a `.scad` extension will open them
in in OpenSCAD.
...
combining code execution and document. We won't cover them here, but they're
worth checking out.
...
2. Enter the import lines at the beginnig of `solution.py` one at a time
in the interpreter (these are `from digifab import *` and `import numpy, 
solid`). Since these lines include the packages you will use most for this
class it's a good idea to get in the habit of doing this first when using
iPython interactively.

3. Enter the first line of code from the `example` function in `solution.py` 
(`open_pl = ... `) into the interpreter, and press enter. View the `open_pl`
PolyLine by entering `open_pl.show()` in the interpreter. You will have to 
close the viewing window before entering more code. You can also dump all of
the data in `open_pl` by just typing that and hitting enter in the interpreter.

4. Enter the rest of the `example` function lines one at a time (copy paste 
...
`print solid.scad_render(squarcle_gen)` will show the OpenSCAD string generated
by `squarcle_gen`. 

*Question 6: What OpenSCAD function correlates to the minus sign used to make
`squarcle_gen`?*

`len(squarcle_pl)` will return the number of unconnected polylines in the 
PolyLine object.

*Question 7:  How many polylines were in the PolyLine created with 
`squarcle_gen` before `simplified()` was called on it?*

*Question 8: Can an integer be used to scale a PolyLine object with the
multiplication notation?*

Python uses so-called "magic functions" to implement the behavior
...
object by typing entering `obj.__` and pressing tab. You can then add the 
question mark to any of these (e.g. `open_pl.__add__?`) to get the docstring
for that function. 

*Question 9: Which magic function and symbol has the equivalent behavior
of the `unioned` function defined for the PolyLine class?*
...
PolyLine class (other than constructors). For any PolyLine, you can get an
OpenSCAD representation by calling it's `get_generator()` function.
